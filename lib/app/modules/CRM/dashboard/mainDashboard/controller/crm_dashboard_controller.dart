  ...
  import 'package:construction_technect/app/modules/CRM/dashboard/mainDashboard/services/crm_dashboard_service.dart';

  class CRMDashboardController extends GetxController {
    final CRMDashboardService _dashboardService = CRMDashboardService();
    ...
    void updateDashboardUI() {
      ...
      if (data.funnelData.isNotEmpty) {
        funnelData.value = data.funnelData
            .map((f) => {'label': f.label, 'count': f.count, 'color': _parseColor(f.color)})
            .toList();
      }

      if (data.productChart.isNotEmpty) {
        productA.value = data.productChart.map((p) => p.productA.toDouble()).toList();
        productB.value = data.productChart.map((p) => p.productB.toDouble()).toList();
      }
    }
    ...
    bool canShowCurrentTab(CRMDashboardController c) {
      if (!myPref.getIsTeamLogin()) return true;
      if (c.totalMarketing.value) {
        return PermissionLabelUtils.canShow(PermissionKeys.marketingLeadManager);
      }
      if (c.totalSales.value) {
        return PermissionLabelUtils.canShow(PermissionKeys.salesLeadManager);
      }
      if (c.totalAccounts.value) {
        return PermissionLabelUtils.canShow(PermissionKeys.accountLeadManager);
      }
      return false;
    }
    bool get hasAnyDashboardPermission {
      if (!myPref.getIsTeamLogin()) return true;

      return PermissionLabelUtils.canShow(PermissionKeys.marketingLeadManager) ||
          PermissionLabelUtils.canShow(PermissionKeys.salesLeadManager) ||
          PermissionLabelUtils.canShow(PermissionKeys.accountLeadManager);
    }
